...
Speed Speed Speed
Mater fitter does parallel processeing (you are welcome!) Best case have one core per data set + 1 more to handel OS calls
Two core functions a0_alpha and a0_bainite are written in C which provides orders of magnitude speed up. master_fitter_utils is the C wrapper of these two functions.
...
A program caller data_plotter is written to creating the master_setup_file. This part is still a little tricky. follow these steps:
...
Sources of error:
    ...
    2. Initial guess of N_t is too big or small.
...
#each line in the setup file is like this:
#filename,Aus_min_row,Aus_max_row,fer_min_row,fer_max_row,L0,cementite precipitation
...
################################
#solver settings
end_fit_WF=5# this make sure that the product(alpha) range fitts well.
overal_fit_WF=4 # this is the weight factor for overal fit overal. Try not changing this.
err_end_slop_WF=6 #This weight factor makes sure that optimized parameters result in a simulation of dilation that has 0 df/dt(rate of transformation) in the product range.
err_maximum_transformation_WF=1 # This weight fraction maximizes the simulated fractions of transformation.
#use maximization with caution. this is a tool to get close to the final solution. However if your final solution relies on maximization to be on, your solution might not be correct. try reducing some of the other weight factors.
#having said that, using err_maximum_transformation_WF does not make your calculations wrong.

maximize_fraction_transformed='yes' #'yes' and 'y' will trigger maximizing transformation weight factor. Be careful with use of this. It may create incorrect lattice parameters.
sr=10
reg_order=3
interval=3
optimize='yes'# It tells the program to optimize or just plot using the parameters provided.yes / no
Bs_model='empirical'# Do not chnage this unless you have a better model for prediction of Bs.
...
optimized_param={'a0_gama':0, 'a0_alpha':1,'CTE_alpha_a':1,'CTE_alpha_b':1,
                'CTE_alpha_c':1,'c_wf_for_cte':1, 'c_wf_for_a0':1}

################################

################################
#starting values of the fundamental parameters of the program.
a0_gama =     3.639013224043719782910143775695e-10
a0_alpha=     2.868001781715644648539242589033e-10
CTE_alpha_a=  -8.737494727282760867869786431295e-12
CTE_alpha_b=  5.327075182648812447019802320414e-09
CTE_alpha_c=  1.388036482119440876738247769850e-05
c_wf_for_cte= 6.013939018645183751110572245580e-04
c_wf_for_a0=  8.125071603526352080620103100605e-12

#a0_gama =     3.639012463394334741955289099440e-10
#a0_alpha=     2.868484466850418685186706145704e-10
#CTE_alpha_a=  -9.543227020737187133986278041563e-12
#CTE_alpha_b=  6.313653881291318607830537158396e-09
#CTE_alpha_c=  1.252382284198925952996110655402e-05
#c_wf_for_cte= 4.158147723081937700145838121557e-04
#c_wf_for_a0=  1.033554631555363963087284213760e-11
#for interval of 2
#a0_gama =     3.639013224043722367849558003907e-10
#a0_alpha=     2.868326931233762148203647829401e-10
#CTE_alpha_a=  -8.736541721268103617671938524762e-12
#CTE_alpha_b=  5.569760572282707328088656743761e-09
#CTE_alpha_c=  1.366396427991553046599366993963e-05
#c_wf_for_cte= 6.411332870763662619001399534113e-04
#c_wf_for_a0=  6.959413252658559283273728965319e-12
#################################   
...
#        print Vgama_param(a0_gama,CTE_0_gama,temp_analysis_aus)
#        print modeled_l_gama
...
#            dn_cem=((N_total-N_product[i-1])*(L(C_in_aus_molefraction[i-1])-L(Solubility_aus_cement(temp_fit[i])))-x*(L(C_dn[i-1])-L(Solubility_aus_cement(temp_fit[i]))))/(L(0.25)-L(Solubility_aus_cement(temp_fit[i])))
    ...
    def Ms(C):# C is in mole fraction ref http://www.lucefin.com/en/siderurgia/area-tecnica/trattamenti-termici/
        C=mf_to_wp(C)
        Ms=539-423*C-30.6*Mn-16.6*Ni-12*Cr-7.5*Mo-11.3*Cu-14.5*Si 
        return Ms
