...
About 20% increase in speed is achieved this way. Functiones like Ms, Bs, L ...
which are shared between V10 and V10_plot are defined outside which reduces the risk of 
V10 and V10_plot being different. Vcement is imported as a C library for speed.
Dataset conditioner is improved to V3 which is slighly more consistent is picking 
analysis points. 
...
Speed Speed Speed
Mater fitter does parallel processeing (you are welcome!) Best case have one core per data file + 1 more to handel OS calls. 
...
This version can opimize the weight factor of C on CTE of martensite and the weight 
factor of C on expantion of martensite along with some more which can be found in the code. 
...
Sources of error:
    ...
    3. "Can not import Valpha, Vcement..." you may have to recompile the cython 
...
#    import winsound
from Results_xlsx_save_v2 import saver
#import data_set_conditioner_V2
import data_set_conditioner_V3
...
overal_fit_WF=6
err_end_slop_WF=0
err_maximum_transformation_WF=3.0
...
normalizing_temp=940    # Only applicapable for method 3 of L0_correction.
...
SSF_method="user" # can be user or genral. general assumes  linear increase of SSF from LBs to Ms, from SSF=0 to SSF=1, User defined is polynomial. 
...
a0_alpha=     0.000000000286303685572245
CTE_alpha_a=  0.000000000000000000000000000000e+00
CTE_alpha_b=  0.00000000482223600080234
CTE_alpha_c=  0.0000127339731685667
c_wf_for_cte= 0.000251734890694533
c_wf_for_a0=  0.0000000000026039264325498
...
CSLA_low=P.polyfit(T_CSLA[BB:],X_CSLA[BB:],4)

#@jit(float64(float64))
def Solubility_aus_cement(T):#This function is not vectorizable because of if statements.
    if T<T_CSLA[BB]:
        if T<T_CSLA.min():
            CSLA=0
        else:
...
ax1.plot(T_CSLA[2:-2],scipy.misc.derivative(Solubility,T_CSLA[2:-2],dx=0.1,n=2),'o',label='2nd derivative')
ax1.set_xlabel('Temperature')
ax1.set_ylabel('2nd derivative')
ax1.set_title('Solubility limit of C in austenite')
ax1.legend(loc='upper right')
ax2=ax1.twinx()
ax2.plot(T_CSLA,Solubility(T_CSLA),'o',label='Raw solubility data')
ax2.set_ylabel('Solubility limit mole fraction')
ax2.plot(T_CSLA,CSLA,'--',label='Fitted function')
...
def mf_to_wp(MF):
#    y=0
#    for i in xrange(len(MF_to_WP)):
#        y = y + MF ** i * MF_to_WP[i]
#    return y
#    if type(MF).__name__ != ('ndarray' or 'list'): #this try except allows mf_to_wp function to operated on single MF and on numpy arrays at the same time.
    try:#This will allow handelling numpy arrays
#        strMF=("{0:.%df}"%(precision)).format(MF)
#        strMF=np.float16(MF)
        strMF=str(MF)[:(precision+3)]
#        if strMF in MF_dic:
...
plt.figure(23)
plt.plot(CR_master,np.array(CTE_aus_master)*10e5,'-o')
plt.title('CTE of austenite vs cooling rate')
plt.xlabel('Cooling rate(C/s)')
plt.ylabel('CTE (*10e-5)')    
...
for i in range(len(file_list)):
    ...
    plt.plot(temp,CTE,'.',label=file_list[i][0])
...
plt.plot(range(100,500),P.polyval(range(100,500),CTE_product_avr_coef),"--",label='Average CTE')
plt.legend()
...
def Bainite_ss_factor(T,C): #using relaxation observed in isothermal experiments, C is the amount of C in Austenite.
    if SSF_method.lower()=="general":
        SSF_coef=P.polyfit([Bs(C)-100,Ms(C)],[0,1],1) #this has to fixed. lower Bs has to be used not just Bs which is for upper bainite.
...
output = mp.Queue()
def Fitter(filename,output,a0_gama,CTE_alpha_a,CTE_alpha_b,CTE_alpha_c,a0_alpha,\
    c_wf_for_cte,Bs_master_dic, MF_dic, Ms_master_dic, C_in_alpha_master_dic):
    global sr, reg_order, interval, Bs_model, end_fit_WF, overal_fit_WF, err_end_slop_WF,\
...
#    CTE_product_coef=P.polyder(P.polyfit(temp_analysis_fer,dil_analysis_fer,1))/L0
...
#    @jit(nopython=True)   
    ...
    x0=[3e22]#initial estimate of x0 (Here it reprisents N_t) is very important. Too big of number will give error.
    for i in xrange(5):
...
#                    print "   Bainite forming according to empirical formula"
                ...
                if ID_dn[i-1][0:2]=="PF":
                    ...
                    if C_dn[i-1]>C_in_gama(i-1,dn_alpha[i-1],dn_alpha,C_dn,N_total,N_product,temp_fit,ID_dn):
                        C_dn[i-1]=C_in_gama(i-1,dn_alpha[i-1],dn_alpha,C_dn,N_total,N_product,temp_fit,ID_dn)
                if ('yes' or 'y') in file_list[k][6].lower() and C_in_gama(i-1,dn_alpha[i-1],dn_alpha,C_dn,N_total,N_product,temp_fit,ID_dn)>Solubility_aus_cement(temp_fit[i-1]):
...
#                    print "   Bainite forming according to empirical formula"
                ...
                if ID_dn[ii][0:2]=="PF":
                    C_dn[ii]=C_in_alpha(T)
                    if C_dn[ii]>C_in_gama(ii,dn_alpha[ii],dn_alpha,C_dn,N_total,N_product,temp_fit,ID_dn):
                        C_dn[ii]=C_in_gama(ii,dn_alpha[ii],dn_alpha,C_dn,N_total,N_product,temp_fit,ID_dn)
                elif ID_dn[ii][0:2]=="BF":
#                    T=(temp_fit[ii]+temp_fit[i])/2
                    C_eq=C_in_alpha(T)
                    C_dn[ii]=Bainite_ss_factor(T,C_in_gama(ii,dn_alpha[ii],dn_alpha,C_dn,N_total,N_product,temp_fit,ID_dn))*(C_in_gama(ii,dn_alpha[ii],dn_alpha,C_dn,N_total,N_product,temp_fit,ID_dn)-C_eq)+C_eq
                    if C_dn[ii]>C_in_gama(ii,dn_alpha[ii],dn_alpha,C_dn,N_total,N_product,temp_fit,ID_dn):
                       C_dn[ii]=C_in_gama(ii,dn_alpha[ii],dn_alpha,C_dn,N_total,N_product,temp_fit,ID_dn)
                if ('yes' or 'y') in file_list[k][6].lower() and C_in_gama(ii,dn_alpha[ii],dn_alpha,C_dn,N_total,N_product,temp_fit,ID_dn)>Solubility_aus_cement(temp_fit[ii]):
                    ID_dn[ii]=ID_dn[ii]+"+"+"Cem"
                elif (("user" in file_list[k][6].lower()) and (C_in_gama(ii,dn_alpha[ii],dn_alpha,C_dn,N_total,N_product,temp_fit,ID_dn)>Solubility_aus_cement(temp_fit[ii]))):
                    LCT=[float(s) for s in file_list[k][7].split() if s.isdigit()][0]#LCT stands for Lowest cementite tempereature
                    if T>LCT:
                        ID_dn[ii]=ID_dn[ii]+"+"+"Cem"
            elif i==1:
                ID_dn.append("PF")
                C_dn[ii]=C_in_alpha((temp_fit[ii]+temp_fit[i])/2)
            else:
                print "There is a problem with ID_dn"
                quit()
...
#                dn_cement[ii]=((N_total-N_product[ii])*(L(C_in_aus_molefraction[ii])-L(Solubility_aus_cement(temp_fit[i])))-dn_alpha[ii]*(L(C_dn[ii])-L(Solubility_aus_cement(temp_fit[i]))))/(L(0.25)-L(Solubility_aus_cement(temp_fit[i])))
            #lets put a constraint on dn to control noise which result in incorrect negative dn
...
#            print i
            ...
            lattice_param_gamma_during_trans[i]=(Vgama_new(CTE_0_gama,C_in_aus[i],temp_fit[i]))**(1/3.0)
...
def master_fiter(x):
    ...
    processes = [mp.Process(target=Fitter, args=((file_list[k][0],output,a0_gama,CTE_alpha_a,\
    CTE_alpha_b,CTE_alpha_c,a0_alpha,c_wf_for_cte,Bs_master_dic, MF_dic, Ms_master_dic,C_in_alpha_master_dic ))) for k in range(len(file_list))]
...
#    Ms_master_dic=mp.Manager().dict()
    ...
    for i in range(len(result_master_time)):
        plt.plot(result_master_temp[i][10:-10],result_master_lattice_param_gamma_during_trans[i][10:-10], '-',label=file_list[i][0])
    plt.legend(loc=4)
    plt.xlabel('Temperature (C)')
    plt.ylabel('Gamma lattice parameter(m)')
    plt.grid()      
    
    plt.figure(4)
    for i in range(len(result_master_time)):
        plt.plot(result_master_C_in_gama[i][10:-10],result_master_lattice_param_gamma_during_trans[i][10:-10], '-',label=file_list[i][0])
    plt.legend(loc=4)
