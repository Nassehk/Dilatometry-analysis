...
import subprocess
from numba import autojit, jit, float64, float32, vectorize
start=time.time()
...
normalizing_temp=900    # Only applicapable for method 3 of L0_correction.
...
#                        starting point. It is faster but pron to pointing to a 
...
a0_alpha=     2.862879955016251874727730369159e-10
CTE_alpha_a=  0.000000000000000000000000000000e+00
CTE_alpha_b=  4.930999380801883527397038288562e-09
CTE_alpha_c=  1.290878656293724887379852084290e-05
c_wf_for_cte= 2.517523737198491188930826734094e-04
c_wf_for_a0=  2.103821561433660701283252659531e-12
...
for row in file_list:
    ...
    if CR[0]=='0':
        ...
        while CR_split[i].isdigit()==False:
            i+=1
#            at this point we i shows the number of zeros are in front of the 
...
def corr_fact_l0(x):
    ...
    if L0_correction_method==3:
        err=(5+abs(strain_at_normalizing_temp_master[m]-strain_at_normalizing_T_avr+x))**5-5**5
